...
class RomanticTravelPDF(FPDF):
    def header(self):
        if self.page_no() == 1:
            return
        if os.path.exists("pulsating_hearts.gif"):
            self.image("pulsating_hearts.gif", x=0, y=0, w=210, h=297)
...
pdf.add_page()
if os.path.exists("pulsating_hearts.gif"):
    pdf.image("pulsating_hearts.gif", x=0, y=0, w=297)
pdf.ln(160)
pdf.set_font("DejaVu", "B", 25)
pdf.set_text_color(180, 0, 89)  # romanttinen pinkinpunainen
pdf.multi_cell(0, 10,
